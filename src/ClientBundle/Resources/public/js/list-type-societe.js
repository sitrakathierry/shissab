...
$(document).ready(function(){
	...
	function instance_grid() {
        ...
        var tableau_grid = $('#list_type_societe');

        if (tableau_grid[0].grid == undefined) {
            tableau_grid.jqGrid(options);
        } else {
            delete tableau_grid;
            $('#list_type_societe').GridUnload('#list_type_societe');
            tableau_grid = $('#list_type_societe');
            tableau_grid.jqGrid(options);
        }
    ...
    function load_list() {
        ...
        var url = Routing.generate('typesociete_get_list')
        var data = {};
	...
	$(document).on('click','#btn-save',function(event) {
        ...
        if(enregistre)
        {
            ...
            var url = Routing.generate('typesociete_save');
            ...
            $.ajax({
                ...
                data: data,
                success: function(res) {
                    show_info('SUCCESS','Type de société enregistré');
                    load_list();
                    // body...
                }
            })
	...
	$(document).on('click', '.cl_edit_type', function(){
        ...
        if (action === 2) {
            ...
            swal({
                title: "SUPPRIMER",
                text: "Voulez-vous vraiment supprimer le type de société?",
                type: "warning",
                ...
                var url = Routing.generate('typesociete_delete',{
                    id : id
                });
                ...
                $.ajax({
                    ...
                    datatype: 'json',
                    success: function(res) {
                        swal("Supprimé!", "Le type de société a été supprimé", "success");
    ...
    function show_editeur_marque(id)
    {
        ...
        $.ajax({
            ...
            type: 'POST',
            url: Routing.generate('typesociete_editor'),
            dataType: 'html',
    ...
    function edit_type(act, data)
    {
        $.ajax({
            data: data,
            type: 'POST',
            url: Routing.generate('typesociete_save'),
            dataType: 'html',
            success: function(data) {
