...
$(document).ready(function(){
	...
	function instance_grid() {
        var colNames = ['Catégorie','Type','Nom', 'Prix achat', 'Prix vente','Statut',''];
        ...
        var tableau_grid = $('#liste_plat');

        if (tableau_grid[0].grid == undefined) {
            tableau_grid.jqGrid(options);
        } else {
            delete tableau_grid;
            $('#liste_plat').GridUnload('#liste_plat');
            tableau_grid = $('#liste_plat');
            tableau_grid.jqGrid(options);
        }
    ...
    function load_list() {
        ...
        var url = Routing.generate('restaurant_plat_list');
        var data = {
        	agence : $('#agence').val(),
	...
	$(document).on('click', '.afficher', function(){
        ...
        window.location.href = Routing.generate('restaurant_plat_show', { id : id })
    });
