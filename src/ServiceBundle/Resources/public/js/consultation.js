...
$(document).ready(function(){
	...
	function instance_grid() {
        ...
        var tableau_grid = $('#liste_service');

        if (tableau_grid[0].grid == undefined) {
            tableau_grid.jqGrid(options);
        } else {
            delete tableau_grid;
            $('#liste_service').GridUnload('#liste_service');
            tableau_grid = $('#liste_service');
            tableau_grid.jqGrid(options);
        }
    ...
    function load_list() {
        ...
        var url = Routing.generate('service_list');
        var data = {
        	agence : $('#agence').val(),
	...
	$(document).on('click', '.afficher', function(){
        ...
        window.location.href = Routing.generate('service_show', { id : id })
    });
