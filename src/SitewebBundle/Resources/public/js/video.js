...
$(document).ready(function(){
	...
	function instance_grid() {
        ...
        },
        {
            ...
            formatter: function(v){ 
                return '<button class="btn btn-xs btn-outline btn-primary edit_video " data-type="0"><i class="fa fa-pencil-square-o "></i>&nbsp;Afficher</button> &nbsp;'; 
            }
        }];
        ...
        var tableau_grid = $('#list_video');

        if (tableau_grid[0].grid == undefined) {
            tableau_grid.jqGrid(options);
        } else {
            delete tableau_grid;
            $('#list_video').GridUnload('#list_video');
            tableau_grid = $('#list_video');
            tableau_grid.jqGrid(options);
        }
    ...
    function load_list() {
        ...
        var url = Routing.generate('siteweb_video_list')
        var data = {
        	id_siteweb : $('#id_siteweb').val(),
        };
    ...
    $(document).on('click', '.edit_video', function(){
        ...
        location.href = Routing.generate('siteweb_video_show', { id : id })
