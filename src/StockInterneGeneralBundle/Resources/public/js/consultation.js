...
$(document).ready(function(){
	...
	function instance_grid() {
        ...
        var tableau_grid = $('#liste_stock_interne_general');

        if (tableau_grid[0].grid == undefined) {
            tableau_grid.jqGrid(options);
        } else {
            delete tableau_grid;
            $('#liste_stock_interne_general').GridUnload('#liste_stock_interne_general');
            tableau_grid = $('#liste_stock_interne_general');
            tableau_grid.jqGrid(options);
        }
    ...
    function load_list() {
        ...
        var url = Routing.generate('stock_interne_general_list');
        var data = {
        	agence : $('#agence').val(),
	...
	$(document).on('click', '.afficher', function(){
        ...
        window.location.href = Routing.generate('stock_interne_general_show', { id : id })
    });
