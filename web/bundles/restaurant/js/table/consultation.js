...
$(document).ready(function(){
	...
	function instance_grid() {
        var colNames = ['Nom', 'Nb place', 'Disponibilité','Statut',''];
        ...
        var tableau_grid = $('#liste_table');

        if (tableau_grid[0].grid == undefined) {
            tableau_grid.jqGrid(options);
        } else {
            delete tableau_grid;
            $('#liste_table').GridUnload('#liste_table');
            tableau_grid = $('#liste_table');
            tableau_grid.jqGrid(options);
        }
    ...
    function load_list() {
        ...
        var url = Routing.generate('restaurant_table_list');
        var data = {
        	agence : $('#agence').val(),
	...
	$(document).on('click', '.afficher', function(){
        ...
        window.location.href = Routing.generate('restaurant_table_show', { id : id })
    });
